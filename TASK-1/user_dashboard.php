...
if (isset($_POST['pay_bill'])) {
    $bill_no = sanitize_input($_POST['bill_no']);
    $sql = "UPDATE bills SET status = 'Paid', payment_date = NOW() WHERE bill_no = ? AND service_no = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ss", $bill_no, $service_no);
    $stmt->execute();
}
